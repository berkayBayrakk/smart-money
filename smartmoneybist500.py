...
def run_titan_prod():
    ...
    xu500 = yf.Ticker("XU500.IS")

    # Yahoo bazen bunu dict / list döndürebilir
    TICKERS = list(xu500.constituents)

    # Güvenlik: sadece hisse kalsın
    TICKERS = [t for t in TICKERS if t.endswith(".IS")]
    raw_data = yf.download(TICKERS, period="2y", group_by='ticker', auto_adjust=False, progress=False)
    results = []

    for t in TICKERS:
        if t == "XU500.IS": continue
        try:
            try:
